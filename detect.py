...
def main():
    ...
    imgs = data['x']
    prob_noise = data['noise']

    for index1, vec in enumerate(imgs):

        f, axes = plt.subplots(2, vec.shape[0], figsize=(30,10))
        
        for index2, img in enumerate(vec): 
            ...
            for i, t in enumerate([0.94]):
                ...
                for (a, b), s in zip(nlines, nscores):
                    if s < t:
                        continue
                    axes[1, index2].plot([a[1], b[1]], [a[0], b[0]], c=c(s), linewidth=2, zorder=s)
                    axes[1, index2].scatter(a[1], a[0], **PLTOPTS)
                    axes[1, index2].scatter(b[1], b[0], **PLTOPTS)
                # plt.gca().xaxis.set_major_locator(plt.NullLocator())
                # plt.gca().yaxis.set_major_locator(plt.NullLocator())
                axes[1, index2].imshow(im)
                axes[0, index2].imshow(im)
                axes[0, index2].title.set_text('intensity={}'.format(prob_noise[index2]))
                axes[1, index2].title.set_text('scores={}'.format(scores.round(2)))
        plt.savefig('data/intensity/{}.png'.format(index1), bbox_inches="tight")
        # plt.show()
        plt.close()
